...
class MemoryManager:
    ...
    PROFILE_DEBOUNCE_DELAY = 3.0  # 画像更新防抖延迟（秒）
    PROFILE_MIN_NEW_MESSAGES = 4  # 最少累积新消息数才更新画像
    ...
    def __init__(
        ...
        # 画像更新防抖和增量状态
        self._profile_pending_tasks = {}  # 防抖：存储待执行的更新任务
        self._profile_last_processed = {}  # 增量：记录每个会话已处理的消息数量
    ...
    async def update_profile(self, user_id: str, conv_id: str) -> None:
        ...
        优化策略：
        1. 防抖（Debounce）：延迟 3 秒执行，合并短时间内的多次触发
        2. 增量（Incremental）：只处理自上次更新后的新消息，避免重复计算
        3. 条件过滤：新消息太少（< 4 条）或内容太短时跳过更新
        ...
        # 取消之前的防抖任务（如果存在）
        if key in self._profile_pending_tasks:
            self._profile_pending_tasks[key].cancel()

        # 创建新的延迟任务
        async def delayed_update():
            await asyncio.sleep(self.PROFILE_DEBOUNCE_DELAY)
            await self._do_update_profile(user_id, conv_id)

        self._profile_pending_tasks[key] = asyncio.create_task(delayed_update())

    async def _do_update_profile(self, user_id: str, conv_id: str) -> None:
        """实际执行画像更新（增量处理）。"""
        user_id = self._safe_text(user_id)
        conv_id = self._safe_text(conv_id)
        key = f"{user_id}:{conv_id}"
        ...
        # 2. 增量处理：只处理新消息
        last_processed_count = self._profile_last_processed.get(key, 0)
        new_messages = messages[last_processed_count:]

        # 3. 过滤条件：新消息太少不更新
        if len(new_messages) < self.PROFILE_MIN_NEW_MESSAGES:
            logger.debug(f"画像更新跳过: {user_id} 新消息不足 ({len(new_messages)} < {self.PROFILE_MIN_NEW_MESSAGES})")
            return

        # 4. 过滤条件：内容太短不更新
        new_text = "\n".join(f"{m.role.value}: {m.content}" for m in new_messages)
        new_text = self._safe_text(new_text)
        if len(new_text) < 50:
            logger.debug(f"画像更新跳过: {user_id} 新内容太短 ({len(new_text)} 字符)")
            return

        # 5. 读取旧画像
        current_profile = await self._get_profile(user_id)
        profile_ctx = json.dumps(current_profile, ensure_ascii=False) if current_profile else "空"

        # 6. 改进的 Prompt：只处理新对话，增量合并
        prompt = f"""你是用户画像分析专家。请从新对话中提炼用户偏好和关键实体，与已有画像增量合并。
...
2. 合并规则：
   - 保留已有画像中仍然有效的信息
   - 如果新对话显示偏好改变（如从"喜欢详细"变为"喜欢简洁"），用新偏好替换旧偏好
   - 实体信息优先保留最新的（如最新的订单号）
   - 去重：相同或相似的偏好只保留一个
...
【示例】
已有画像: {{"preferences": ["关注退款问题"], "entities": {{"订单号": ["12345"]}}}}
新对话: 用户询问订单 67890 的物流，希望简洁回答
输出: {{"preferences": ["关注退款问题", "关注物流", "喜欢简洁回答"], "entities": {{"订单号": ["67890"], "问题类型": ["退款", "物流"]}}}}
        ...
        try:
            ...
            await asyncio.to_thread(
                ...
                metadatas=[{
                    ...
                    "processed_messages": len(messages),  # 记录处理的消息总数
                }],
            )

            # 9. 更新增量状态
            self._profile_last_processed[key] = len(messages)

            logger.info(f"用户画像已更新: {user_id} (处理 {len(new_messages)} 条新消息，累计 {len(messages)} 条)")
            ...
            # 10. 清理防抖任务
            if key in self._profile_pending_tasks:
                del self._profile_pending_tasks[key]
