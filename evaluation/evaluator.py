...
严格要求：
1. 只返回纯JSON对象，不要任何其他文字、解释或markdown
2. 必须包含四个字段: relevance, accuracy, completeness, helpfulness
3. 所有值必须是0.0到1.0之间的浮点数
4. 示例: {{"relevance": 0.9, "accuracy": 0.8, "completeness": 0.7, "helpfulness": 0.85}}"""
    ...
    async def judge(
        ...
        try:
            ...
            if s == -1 or e == 0:
                raise ValueError(f"无法从响应中提取JSON: {raw[:100]}")
